...
public class TGS implements Runnable{
	...
	public void run() {		
		...
		try {
			...
			byte[] bt = new BASE64Decoder().decodeBuffer(ticketTgsEncipher);
			...
			byte[] bt1 = new BASE64Decoder().decodeBuffer(AuthenticatorEncipher);
			...
			String ticketPServerEncipher = new BASE64Encoder().encodeBuffer(temp1);
	...
	private class BASE64Decoder {
		public byte[] decodeBuffer(String authenticatorEncipher) {
			return new byte[0];
		}}


	private class BASE64Encoder {
		public String encodeBuffer(byte[] temp1) {
			return null;
		}}
	}
